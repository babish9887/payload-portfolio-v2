...
const AboutPage = () => {
  ...
  useEffect(() => {
    ...
    async function getLinks() {
      try {
        const res = await fetch('/api/getcvlink')
        if (!res.ok) {
  ...
  return (
    <div
      id="aboutme"
      className="section w-full flex justify-center items-center lg:p-10 p-5 md:p-8 md:pb-5"
    >
        ...
        <div className="aboutme w-full flex flex-col md:flex-row justify-center items-start gap-y-10 ">
          ...
          <div className="brief  w-full md:w-[50%] text-xl p-5 text-center sm:text-left">
            <p>
              ...
              for optimized performance, and both{' '}
              <span className="text-cyan-400">MongoDB and SQL</span> for robust data management. I
              thrive on tackling new challenges and staying at the forefront of web development.
            </p>
            <button className="mt-4 flex justify-center items-center gap-x-5" onClick={handleClick}>
              Skills{' '}
              <IoIosArrowDown
                className={`transition duration-200 ${skillDropDown ? 'transform rotate-180' : ''}`}
              />
            </button>
            {MySkills.map((skill) => (
              <div key={skill.name} id="skill" className="skill flex flex-col my-2 h-0 opacity-0">
                <p className="text-sm mb-[5px]">{skill.name}</p>
                <span className="bar w-[200px] h-[10px] rounded-md bg-slate-700 overflow-hidden">
                  <span
                    className={`bar_value h-full float-left bg-gradient-to-r from-cyan-500 to-blue-500`}
                  ></span>
                </span>
              </div>
            ))}
...
const AnimatedText = ({ text }: { text: string }) => {
  ...
  return (
    <div
      ref={textRef}
      className="tracking-wider text-5xl line leading-tight font-bold text-center sm:text-left"
    >
      {text.split(' ').map((word, index) => (
        <span
          key={index}
          className={`word inline-block mx-1 ${word == 'Web' || word == 'Developer,' ? 'text-cyan-400' : 'text-white'}`}
        >
          {word}
        </span>
      ))}
